...
async def flow_settings_getter(dialog_manager: DialogManager, **kwargs):
    ...
    flow_data = dialog_data.get("channel_flow") or start_data.get("channel_flow")

    channel_data = dialog_data.get("selected_channel") or start_data.get(
        "selected_channel"
    )
...
async def character_limit_getter(dialog_manager: DialogManager, **kwargs):
    limit_map = {"to_100": "100", "to_300": "300", "to_1000": "1000"}
    if "channel_flow" not in dialog_manager.dialog_data:
        dialog_manager.dialog_data["channel_flow"] = dialog_manager.start_data[
            "channel_flow"
        ]
    return {
        "char_limit": limit_map.get(
            dialog_manager.dialog_data["channel_flow"].content_length
        )
    }
...
async def posts_in_flow_getter(dialog_manager: DialogManager, **kwargs):
    ...
    return {"posts_count": dialog_manager.dialog_data["channel_flow"].flow_volume}


async def get_sources_data(dialog_manager: DialogManager, **kwargs):
    flow_data = dialog_manager.dialog_data.get(
        "channel_flow"
...
async def get_source_type(dialog_manager: DialogManager, **kwargs):
    return {"source_type": dialog_manager.dialog_data.get("new_source_type", "джерела")}


async def get_current_source(dialog_manager: DialogManager, **kwargs):
    try:
        flow = dialog_manager.dialog_data.get(
            "channel_flow"
        ) or dialog_manager.start_data.get("channel_flow")
        ...
        return {"source_type": "Помилка", "source_link": str(e), "source_idx": -1}
...
async def get_sources_list(dialog_manager: DialogManager, **kwargs):
    ...
    for idx, src in enumerate(sources):
        ...
        formatted_sources.append(
            {"type": src["type"], "link": link, "idx": idx, "full_link": src["link"]}
        )
...
async def get_source_to_delete_data(dialog_manager: DialogManager, **kwargs):
    flow = dialog_manager.dialog_data.get(
        "channel_flow", dialog_manager.start_data["channel_flow"]
    )
    ...
    if flow.sources:
        ...
        return []
    return {"source_to_delete": source_to_delete}
