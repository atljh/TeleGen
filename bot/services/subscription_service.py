...
from bot.database.exceptions import SubscriptionNotFoundError, ChannelNotFoundError, UserNotFoundError
...
class SubscriptionService:
    ...
    async def get_subscription_by_id(self, subscription_id: int) -> SubscriptionDTO:
        try:
            ...
            raise SubscriptionNotFoundError(f"Подписка с id={subscription_id} не найдена.")
    ...
    async def update_subscription(
        ...
        try:
            ...
            updated_subscription = await self.subscription_repository.update_subscription(subscription)
    ...
    async def get_user_subscriptions(self, user_id: int) -> List[SubscriptionDTO]:
        try:
            user = await self.user_repository.get_user_by_id(user_id)
            subscriptions = await self.subscription_repository.get_subscriptions_by_user(user)
    ...
    async def get_channel_subscriptions(self, channel_id: int) -> List[SubscriptionDTO]:
        try:
            channel = await self.channel_repository.get_channel_by_id(channel_id)
            subscriptions = await self.subscription_repository.get_subscriptions_by_channel(channel)
            return [SubscriptionDTO.from_orm(sub) for sub in subscriptions]
        except ChannelNotFoundError:
            raise ChannelNotFoundError(f"Канал с id={channel_id} не найден.")