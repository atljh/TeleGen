...
async def flow_volume_getter(dialog_manager: DialogManager, **kwargs):
    return {
        "current_value": dialog_manager.dialog_data.get(
            "flow_volume", "не встановлено"
        ),
...
async def source_confirmation_getter(dialog_manager: DialogManager, **kwargs):
    ...
    return {"source_type": str(source_type), "source_link": str(source_link)}
...
async def flow_confirmation_getter(dialog_manager: DialogManager, **kwargs):
    ...
    sources = "\n".join(
        f"{src['type']} - {src['link']}" for src in flow_data.get("sources", [])
    )
    ...
    return {
        ...
        "frequency": frequency_map.get(
            flow_data.get("selected_frequency"), flow_data.get("selected_frequency")
        ),
        "words_limit": words_limit_map.get(
            flow_data.get("selected_words_limit"), flow_data.get("selected_words_limit")
        ),
