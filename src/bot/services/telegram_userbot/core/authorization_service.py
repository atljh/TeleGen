...
class AuthorizationService:
    def __init__(self, phone: str | None = None, bot: Bot = None):
        self.phone = phone
        self.bot = bot
        self.logger = init_logger(bot)
    ...
    async def is_authorized(self, client: TelegramClient) -> bool:
        try:
            ...
            logging.error(f"Authorization check failed: {e!s}")
            return False
    ...
    async def _handle_password_needed(self):
        ...
        if self.logger:
            await self.logger.error_occurred(
                error_message="Two-factor authentication required for Userbot!",
                context={"Event": "2FA Required"},
            )
        else:
    ...
    async def _handle_eof_error(self):
        ...
        if self.logger:
            await self.logger.error_occurred(
                error_message="Telegram Userbot requires reauthorization!",
                context={"Event": "EOF / Session Expired"},
            )
        else:
    ...
    async def _handle_authorization_error(self, error: Exception):
        ...
        if self.logger:
            await self.logger.error_occurred(
                error_message="Telegram Userbot authorization error: {error!s}",
                context={"Event": "Auth Error"},
            )
        else:
    ...
    async def send_code_request(self, client: TelegramClient) -> str | None:
        try:
            ...
            logging.error(f"Code request failed: {e!s}")
            return None
    ...
    async def sign_in_with_code(self, client: TelegramClient, code: str) -> bool:
        try:
            ...
            logging.error(f"Sign in with code failed: {e!s}")
            return False
    ...
    async def sign_in_with_password(
        ...
        try:
            ...
            logging.error(f"Sign in with password failed: {e!s}")
            return False
