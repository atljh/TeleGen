...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--min-channels", type=int, default=1, help="Minimum channels per user"
        )
        parser.add_argument(
            "--max-channels", type=int, default=3, help="Maximum channels per user"
        )
        parser.add_argument(
            "--min-flows", type=int, default=1, help="Minimum flows per channel"
        )
        parser.add_argument(
            "--max-flows", type=int, default=2, help="Maximum flows per channel"
        )
        parser.add_argument(
            "--min-sources", type=int, default=2, help="Minimum sources per flow"
        )
        parser.add_argument(
            "--max-sources", type=int, default=5, help="Maximum sources per flow"
        )
        parser.add_argument(
            "--cleanup",
            action="store_true",
